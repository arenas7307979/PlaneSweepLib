...
double cameraDirAngleCosine(PSL::CameraMatrix<double> const &cam1,
                            PSL::CameraMatrix<double> const &cam2) {
  Eigen::Vector4d dirHomo =
      cam1.unprojectPoint(cam1.getK()(0, 2), cam1.getK()(1, 2), 1);
  dirHomo /= dirHomo(3);
  Eigen::Vector3d dir1(dirHomo(0), dirHomo(1), dirHomo(2));
  dir1 -= cam1.getC();
  dir1.normalize();

  Eigen::Vector4d dirHomo2 =
      cam2.unprojectPoint(cam2.getK()(0, 2), cam2.getK()(1, 2), 1);
  dirHomo2 /= dirHomo2(3);
...
int main(int argc, char *argv[]) {
  ...
  desc.add_options()("help", "Produce help message")(
      "frameFolder", boost::program_options::value<std::string>(&frameFolder)
                         ->default_value("capture_1/"),
      "Folders containing the frames.")(
      "krtdFolder", boost::program_options::value<std::string>(&krtdFolder)
                        ->default_value("ba_output/krtd/"),
      "Folders containing the .krtd files.")(
      "landmarksPLY", boost::program_options::value<std::string>(&landmarksPLY)
                          ->default_value("ba_output/landmarks.ply"),
      "File containing the landmark points (to estimate where dense estimation "
      "should take place).")(
      "imageListFile",
      boost::program_options::value<std::string>(&imageListFile)
          ->default_value("frames_capture_1_1in3_for_psl.txt"),
      "Image list file")
      //            ("configFile",
      //            boost::program_options::value<std::string>(&configFile)->default_value("conf3D.txt"),
      //            "Config file")
      ("display", boost::program_options::bool_switch(&display),
       "Display images and depth maps")(
          "outputDirectory",
          boost::program_options::value<std::string>(&outDir)->default_value(
              "depthMaps_capture_1_tmp"),
          "Depth maps output directory")(
          "filter", boost::program_options::bool_switch(&filter),
          "Filter the depths maps based on cost and uniqueness ratio")(
          "debug", boost::program_options::bool_switch(&debug),
          "Show more debug informations")(
          "view", boost::program_options::value<int>(&view)->default_value(-1),
          "View id to generate depthmap for. All if -1.")(
          "PS_MIN_DEPTH",
          boost::program_options::value<float>(&pSMinDepth)->default_value(0.0),
          "Min depth")(
          "PS_MAX_DEPTH",
          boost::program_options::value<float>(&pSMaxDepth)->default_value(0.0),
          "Min depth")(
          "PS_NUM_PLANES",
          boost::program_options::value<int>(&pSNumPlanes)->default_value(100),
          "Number of slices")(
          "PS_RESCALE_FACTOR",
          boost::program_options::value<float>(&pSRescaleFactor)
              ->default_value(1.0),
          "Rescale factor")(
          "PS_MATCHING_COSTS",
          boost::program_options::value<std::string>(&pSMatchingCostsStr)
              ->default_value("SAD"),
          "Type of matching cost strategy [ZNCC or SAD]")(
          "PS_MATCHING_WINDOW_SIZE",
          boost::program_options::value<int>(&pSMatchingWindowSize)
              ->default_value(15),
          "Matching window size")(
          "PS_COLOR_MATCHING",
          boost::program_options::bool_switch(&pSColorMatching),
          "Toggle color matching")(
          "krtdOnly", boost::program_options::bool_switch(&krtdOnly),
          "Don't compute depthmap, just compute scaled krtd")(
          "PS_AUTO_RANGE", boost::program_options::bool_switch(&pSAutoRange),
          "Toggle auto range")(
          "PS_OCCLUSION_MODE",
          boost::program_options::value<std::string>(&pSOcclusionModeStr)
              ->default_value("None"),
          "Type of occlusion mode [None, BestK or RefSplit]")(
          "PS_OCCLUSION_BEST_K_K",
          boost::program_options::value<int>(&pSOcclusionBestKK)
              ->default_value(0),
          "Best K")("PS_USE_SUBPIXEL",
                    boost::program_options::bool_switch(&psEnableSubPixel),
                    "Toggle sub pixel computation")(
          "PS_MAX_NUM_IMAGES",
          boost::program_options::value<int>(&pSMaxNumImages)
              ->default_value(40),
          "Max number of images to use for plane sweep")(
          "PS_COST_THRESHOLD",
          boost::program_options::value<float>(&costThreshold)
              ->default_value(0.0),
          "Cost threshold. Only need if \"filter\" is toggled")(
          "PS_UNIQUENESS_RATIO_THRESHOLD",
          boost::program_options::value<float>(&thresholdUniq)
              ->default_value(0.5),
          "Uniqueness ratio threshold. Only need if \"filter\" is toggled")(
          "PS_MIN_ANGLE_DEGREE",
          boost::program_options::value<float>(&minAngleDegree)
              ->default_value(2.0),
          "Min angle degree between two frames")(
          "ref_view_step",
          boost::program_options::value<int>(&refViewStep)->default_value(1),
          "Compute a depthmap every refViewStep frames.")(
          "writePointClouds",
          boost::program_options::value<std::string>(&writePointClouds)
              ->default_value(""),
          "Saving data to a file [vrml, vti, vtp, vts or vtpvts]")

      //            ("writePointClouds",
      //            boost::program_options::bool_switch(&writePointClouds),
      //            "Write point clouds in vrml format")
      // ("storeBestCostsAndUniquenessRatios",
      // boost::program_options::bool_switch(&storeBestCostsAndUniquenessRatios),
      // "Store the best costs and the uniqueness ratios in dat files")
      ;
  ...
  if (pSMatchingCostsStr == "ZNCC") {
    ...
    PSL_THROW_EXCEPTION(
        "For parameter PS_MATCHING_COSTS only ZNCC or SAD is allowed.")
  }
  ...
  if (pSOcclusionModeStr == "None") {
    ...
    PSL_THROW_EXCEPTION("For parameter PS_MATCHING_COSTS only None or BestK or "
                        "RefSplit is allowed.")
  }
  std::cout << "Matching mode is "
            << (pSOcclusionMode == PSL::PLANE_SWEEP_OCCLUSION_NONE
                    ? "None"
                    : (pSOcclusionMode == PSL::PLANE_SWEEP_OCCLUSION_BEST_K
                           ? "BestK"
  ...
  if (!imagesStream.is_open()) {
    PSL_THROW_EXCEPTION(
        std::string("Could not load images list file : " + imageListFile)
            .c_str())
  }
  ...
  std::cout << numCameras << " filenames found in " << imageListFile
            << std::endl;
  ...
  for (int c = 0; c < numCameras; c++) {
    ...
    std::string krtdMatrixFileName = imageFileNames[c];
    {
      ...
      if (start_pos == std::string::npos)
        PSL_THROW_EXCEPTION(
            ("Could not generate krt filename, no '.' found in " +
             krtdMatrixFileName)
                .c_str())
      krtdMatrixFileName.replace(
          start_pos + 1, krtdMatrixFileName.length() - start_pos, krtdExt);
    }
    ...
    //        {
    //            PSL_THROW_EXCEPTION(("Error opening KRT matrix file :
    //            "+imageListFile).c_str())
    //        }
    ...
    if (krtdMatrixStr.is_open()) {
      ...
      std::cout << "No krtd file found for the frame " << krtdMatrixFile.c_str()
                << ". Jumping to next frame..." << std::endl;
    }
  }
  ...
  std::cout << "Cameras have an average distance of " << avgDistance
            << " and a max distance of " << maxDistance << "." << std::endl;
  ...
  for (std::map<int, PSL::CameraMatrix<double>>::iterator it = cameras.begin();
       it != cameras.end(); std::advance(it, refViewStep), count++) {
    // if we want a specific ref view, go for it.
    if (view >= 0 && it->first != view)
      continue;
    const int refViewId = it->first;
    timer.restart();
    std::cout << "Find overlaps for ref cam: " << refViewId << "..."
              << std::endl;
    std::vector<std::pair<int, float>> overlaps;
    std::cout << "WARNING Overlap prior is small angle for now." << std::endl;
    for (int i = 0; i < cameras.size(); ++i) {
      if (i != it->first)
        overlaps.push_back(std::make_pair(
            i, cameraDirAngleCosine(cameras[refViewId], cameras[i])));
    }
    ...
    timer.restart();
    std::cout << "Setting up cuda plane sweep and upload images..."
              << std::endl;
    PSL::CudaPlaneSweep cPS;
    cPS.setScale(pSRescaleFactor);
    if (debug)
      std::cout << "  pSRescaleFactor " << pSRescaleFactor << std::endl;
    cPS.setMatchWindowSize(pSMatchingWindowSize, pSMatchingWindowSize);
    if (debug)
      std::cout << "  pSMatchingWindowSize " << pSMatchingWindowSize
                << std::endl;
    cPS.setNumPlanes(pSNumPlanes);
    if (debug)
      std::cout << "  pSNumPlanes " << pSNumPlanes << std::endl;
    cPS.setOcclusionMode(pSOcclusionMode);
    if (debug)
      std::cout << "  pSOcclusionMode " << pSOcclusionMode << std::endl;
    if (pSOcclusionMode == PSL::PLANE_SWEEP_OCCLUSION_BEST_K) {
      cPS.setOcclusionBestK(pSOcclusionBestKK);
      if (debug)
        std::cout << "  pSOcclusionBestKK " << pSOcclusionBestKK << std::endl;
    }
    cPS.setMatchingCosts(pSMatchingCosts);
    if (debug)
      std::cout << "  pSMatchingCosts " << pSMatchingCosts << std::endl;
    ...
    if (debug)
      std::cout << "  pSColorMatching " << pSColorMatching << std::endl;

    if (pSAutoRange) {
      // Compute near and far range from points that are in front of the camera
      // and projects within image
      double nearZ, farZ;
      std::vector<float> zs;
      ...
      std::cout << "Image " << it->first << " is assumed to be " << imageWidth
                << "x" << imageHeight << std::endl;
      for (unsigned int i = 0; i < points.size(); i++) {
        Eigen::Vector3d x = (it->second.getR() * points[i] + it->second.getT());
        Eigen::Vector3d px = 1.0 / x[2] * (K * x);
        if (x[2] > 0 && px[0] > 0 && px[0] < imageWidth && px[1] > 0 &&
            px[1] < imageHeight)
          zs.push_back(x[2]);
      }
      ...
      if (zs.size()) {
        ...
        std::cout << "    distances from camera are " << zs[0] << "->"
                  << zs[zs.size() - 1] << " (" << zs.size()
                  << " visible points)" << std::endl;
        std::cout << "    distances [" << outlierThres * 100 << "%->"
                  << (1.0 - outlierThres) * 100 << "%] are " << nearZ << "->"
                  << farZ << std::endl;
        ...
        std::cout << "    planes will be " << nearZ << "->" << farZ << " by "
                  << (farZ - nearZ) / pSNumPlanes << " increment" << std::endl;
        pSMinDepth = nearZ;
        pSMaxDepth = farZ;
        cPS.setZRange(nearZ, farZ);
      } else {
        std::cout << "    NO VISIBLE POINTS, demoting depth-range to 2.5-100 * "
                     "[Cameras average distance]: "
                  << minZ << "->" << maxZ << " ." << std::endl;
        pSMinDepth = minZ;
        pSMaxDepth = maxZ;
    ...
    if (debug)
      std::cout << "  Z range :  " << pSMinDepth << "  - " << pSMaxDepth
                << std::endl;
    ...
    if (refImg.empty())
      ...
      // First, write K File
      {
        ...
        kFile << std::endl << std::endl;
        kFile << refCamScaled.getT()(0, 0) << " " << refCamScaled.getT()(1, 0)
              << " " << refCamScaled.getT()(2, 0) << std::endl;
        kFile << std::endl << std::endl;
        kFile << "0";
        kFile.close();
    ...
    for (int j = 0; j < selectedImg.size(); j++) {
      ...
      if (viewJ == it->first)
        cPSRef = cudaID;
      if (debug)
        std::cout << "Loaded image : " << imageFileName << std::endl;
    }
    std::cout << " Found " << numMatchingImages << " matching images (Angle>"
              << 180.0 / M_PI * acos(minAngleCosine) << " degrees)"
              << std::endl;
    std::cout << " done in " << timer.elapsed() << " seconds." << std::endl;

    if (pSOcclusionMode == PSL::PLANE_SWEEP_OCCLUSION_BEST_K &&
        numMatchingImages < pSOcclusionBestKK) {
      std::cout << "WARNING: skipped reference image " << it->first
                << " because there where not enough matching images for given "
                   "best K, K."
                << std::endl;
      continue;
    }
    if (pSOcclusionMode == PSL::PLANE_SWEEP_OCCLUSION_REF_SPLIT) {
      std::vector<int>::iterator p =
          std::find(selectedImg.begin(), selectedImg.end(), it->first);
    ...
    PSL::DepthMap<float, double> dM;
    {
      ...
      if (filter || storeBestCostsAndUniquenessRatios) {
        std::cout << "Download best costs and uniqueness ratios..."
                  << std::endl;
        ...
        if (storeBestCostsAndUniquenessRatios) {
          ...
          uniqunessFileName << outDir << "/" << baseName
                            << "_uniquenessRatios.dat";
          uniquenessRatios.saveAsDataFile(uniqunessFileName.str());

          float maxBestCost =
              (pSMatchingCosts == PSL::PLANE_SWEEP_SAD)
                  ? (pSMatchingWindowSize * pSMatchingWindowSize) * 255.0f
                  : 1.0f;
          bestCostImgFileName << outDir << "/" << baseName << "_bestCosts.jpg";
          std::cout << "maxBestCost = " << maxBestCost << std::endl;
          PSL::saveGridZSliceAsImage(costs, 0, 0.0f, maxBestCost,
                                     bestCostImgFileName.str().c_str());
          ...
          uniqunessImgFileName << outDir << "/" << baseName
                               << "_uniquenessRatios.jpg";
          PSL::saveGridZSliceAsImage(uniquenessRatios, 0, 0.8f, 1.0f,
                                     uniqunessImgFileName.str().c_str());
        }
    ...
    if (writePointClouds != "") {
      ...
      if (pSRescaleFactor != 1) {
        // scale image
        if (pSRescaleFactor < 1)
          resize(refImg, scaledRefImg, cv::Size(0, 0), pSRescaleFactor,
                 pSRescaleFactor, cv::INTER_AREA);
        else
          resize(refImg, scaledRefImg, cv::Size(0, 0), pSRescaleFactor,
                 pSRescaleFactor, cv::INTER_LINEAR);
      } else {
      ...
      // Convert to VTP and/or VTS file
        ...
        uniquenessRatios->SetNumberOfValues(scaledRefImg.rows *
                                            scaledRefImg.cols);
        ...
        std::string refFrame =
            static_cast<ostringstream *>(
                &(ostringstream() << refViewId / refViewStep))
                ->str();
        std::string depthmapImageFileName =
            outDir + "/" + baseName + "_depth_map." + refFrame + ".vti";
        ...
        //                      realpath(depthmapImageFileName.c_str(),
        //                      depthmapImageNameAbs);
        filenameListvti << refViewId << " " << baseName << "_depth_map."
                        << refFrame << ".vti" << std::endl;
        ...
        if (writePointClouds == "vtp" || writePointClouds == "vtpvts") {
          ...
          std::string depthmapPolyFileName =
              outDir + "/" + baseName + "_depth_map." + refFrame + ".vtp";
          ...
          //                        realpath(depthmapPolyFileName.c_str(),
          //                        depthmapPolyFileNameAbs);
          filenameListvtp << refViewId << " " << depthmapPolyFileName.c_str()
                          << std::endl;
        }
        ...
        if (writePointClouds == "vts" || writePointClouds == "vtpvts") {
          ...
          //                std::string refFrame = static_cast<ostringstream*>(
          //                &(ostringstream() << refViewId/refViewStep)
          //                )->str();
          std::string depthmapGridFileName =
              outDir + "/" + baseName + "_depth_map." + refFrame + ".vts";
          ...
          //                        realpath(depthmapGridFileName.c_str(),
          //                        depthmapGridFileNameAbs);
          filenameListvts << refViewId << " " << depthmapGridFileName.c_str()
                          << std::endl;
        }
      }
    }
    ...
    std::cout << "Elapsed time: " << globalTimer.elapsed() << "s for " << done
              << "/" << tot << " ref views." << std::endl
              << "Estimated Time Remaining: "
              << globalTimer.elapsed() / done * tbd << "s for " << tbd << "/"
              << tot << " ref views." << std::endl;
  }
