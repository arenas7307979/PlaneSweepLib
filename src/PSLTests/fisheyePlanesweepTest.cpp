...
int main(int argc, char *argv[]) {
  ...
  calibrationStr >> K(0, 0);
  calibrationStr >> K(0, 1);
  calibrationStr >> K(0, 2);
  calibrationStr >> K(1, 1);
  calibrationStr >> K(1, 2);
  calibrationStr >> xi;
  calibrationStr >> k1;
  calibrationStr >> k2;
  calibrationStr >> p1;
  ...
  // First tests use the first 5 images
  {
    ...
    cFEPS.setPlaneGenerationMode(
        PSL::FISH_EYE_PLANE_SWEEP_PLANEMODE_UNIFORM_DISPARITY);
    ...
    for (unsigned int i = 0; i < 5; i++) {
      ...
      std::string timestampStr =
          imageFileNames[i].substr(imageFileNames[i].length() - 20, 16);
      uint64_t timestamp = std::atoll(timestampStr.c_str());

      if (timestamp != timestamps[i]) {
        PSL_THROW_EXCEPTION(
            "Sequence broken, timestamps of poses and images do not match.")
      }
...

    {
      ...
      cv::imwrite(
          "fisheyeTestResults/grayscaleSAD/NoOcclusionHandling/refImg.png",
          refImage);
      fEDM.saveInvDepthAsColorImage(
          "fisheyeTestResults/grayscaleSAD/NoOcclusionHandling/invDepthCol.png",
...

    {
      ...
      cv::imwrite("fisheyeTestResults/grayscaleSAD/RefSplit/refImg.png",
                  refImage);
      fEDM.saveInvDepthAsColorImage(
          "fisheyeTestResults/grayscaleSAD/RefSplit/invDepthCol.png",
          (float)minZ, (float)maxZ);
...

    {
      ...
      cv::imwrite(
          "fisheyeTestResults/grayscaleZNCC/NoOcclusionHandling/refImg.png",
          refImage);
      fEDM.saveInvDepthAsColorImage("fisheyeTestResults/grayscaleZNCC/"
...

    {
      ...
      cv::imwrite("fisheyeTestResults/grayscaleZNCC/RefSplit/refImg.png",
                  refImage);
      fEDM.saveInvDepthAsColorImage(
          "fisheyeTestResults/grayscaleZNCC/RefSplit/invDepthCol.png",
          (float)minZ, (float)maxZ);
    ...
    // now the remaining images are added and best K occlusion handling is
    // performed
...

    {
      ...
      fEDM.saveInvDepthAsColorImage(
          "fisheyeTestResults/grayscaleSAD/BestK/invDepthCol.png", (float)minZ,
          (float)maxZ);
...

    {
      ...
      cv::imwrite("fisheyeTestResults/grayscaleZNCC/BestK/refImg.png",
                  refImage);
      fEDM.saveInvDepthAsColorImage(
          "fisheyeTestResults/grayscaleZNCC/BestK/invDepthCol.png", (float)minZ,
          (float)maxZ);
