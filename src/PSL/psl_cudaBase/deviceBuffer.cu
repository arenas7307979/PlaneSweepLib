...
template <typename T> DeviceBuffer<T>::DeviceBuffer() { addr = 0; }
...
void DeviceBuffer<T>::allocatePitched(int width, int height) {
  ...
  PSL_CUDA_CHECKED_CALL(
      cudaMallocPitch(&addr, &pitch, width * sizeof(T), height);)
  this->width = width;
  this->height = height;
}
...
template <typename T> void DeviceBuffer<T>::deallocate() {
  //    if (addr != 0)
  //    {
...
template <typename T> void DeviceBuffer<T>::clear(T value) {

  dim3 gridDim(getNumTiles(width, TILE_WIDTH),
...
void DeviceBuffer<T>::upload(T *dataPtr, size_t dataPitch) {
  // pitch is in bytes
  PSL_CUDA_CHECKED_CALL(cudaMemcpy2D(addr, pitch, dataPtr, dataPitch,
