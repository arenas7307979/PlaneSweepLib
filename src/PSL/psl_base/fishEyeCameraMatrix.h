...
namespace PSL {
template <typename NumericType> class FishEyeCameraMatrix {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  FishEyeCameraMatrix();
  FishEyeCameraMatrix(const Eigen::Matrix<NumericType, 3, 3> &K,
  ...
  void setKRTXi(const Eigen::Matrix<NumericType, 3, 3> &K,
                const Eigen::Matrix<NumericType, 3, 3> &R,
  ...
  Eigen::Matrix<NumericType, 4, 1> unprojectPoint(NumericType x, NumericType y,
                                                  NumericType depth) const;
  Eigen::Matrix<NumericType, 2, 1> projectPoint(NumericType x, NumericType y,
                                                NumericType z) const;
