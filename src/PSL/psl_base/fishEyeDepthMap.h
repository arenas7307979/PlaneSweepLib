...
namespace PSL {
template <typename T, typename U> class FishEyeDepthMap {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  FishEyeDepthMap();
  FishEyeDepthMap(unsigned int width, unsigned int height,
  ...
  void displayColored(T minZ, T maxZ, int displayTime = 0,
                      const char *windowName = "depth map colored");
  void displayInvDepth(T minZ, T maxZ, int displayTime = 0,
                       const char *windowName = "inverse depth map");
  void displayInvDepthColored(
      T minZ, T maxZ, int displayTime = 0,
      const char *windowName = "color coded inverse depth map");
  ...
  void meshToVRML(std::ofstream &os, std::string textureImageFileName,
                  float scale, float maxDispDiff, U maxDist = -1);
  void pointCloudToVRML(std::ofstream &os, U maxDist = -1);
  void pointCloudColoredToVRML(std::ofstream &os, cv::Mat image,
                               U maxDist = -1);
