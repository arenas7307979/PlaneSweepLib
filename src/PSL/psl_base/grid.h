...
namespace PSL {
template <typename Elem> class Grid {
...
public:
  ...
  const Elem &operator()(unsigned int x, unsigned int y,
...
void saveGridZSliceAsImage(Grid<Elem> &grid, unsigned int z,
                           const char *fileName);

template <typename Elem>
void saveGridZSliceAsImage(Grid<Elem> &grid, unsigned int z, Elem minVal,
                           Elem maxVal, const char *fileName);

template <typename Elem>
void displayGridZSliceAsImage(Grid<Elem> &grid, unsigned int z, long time = 0,
                              const char *windowName = "Grid");

template <typename Elem>
void displayGridZSliceAsImage(Grid<Elem> &grid, unsigned int z, Elem minVal,
                              Elem maxVal, long time = 0,
                              const char *windowName = "Grid");
}

template <typename Elem> PSL::Grid<Elem>::Grid() {
  _xDim = 0;
  _yDim = 0;
  _zDim = 0;
  _xyDim = 0;
...
template <typename Elem>
const Elem &PSL::Grid<Elem>::operator()(unsigned int x, unsigned int y,
                                        unsigned int z) const {
  return _cells[z * _xyDim + y * _xDim + x];
}

template <typename Elem>
Elem &PSL::Grid<Elem>::operator()(unsigned int x, unsigned int y,
                                  unsigned int z) {
  return _cells[z * _xyDim + y * _xDim + x];
}

template <typename Elem>
void PSL::Grid<Elem>::resize(unsigned int xDim, unsigned int yDim,
                             unsigned int zDim) {
...
template <typename Elem> unsigned int PSL::Grid<Elem>::getWidth() const {
  return _xDim;
}

template <typename Elem> unsigned int PSL::Grid<Elem>::getHeight() const {
  return _yDim;
}

template <typename Elem> unsigned int PSL::Grid<Elem>::getDepth() const {
  return _zDim;
}

template <typename Elem> unsigned int PSL::Grid<Elem>::getNbVoxels() const {
  return _xyzDim;
}

template <typename Elem> Elem *PSL::Grid<Elem>::getDataPtr() const {
  return _cells.get();
}

template <typename Elem> PSL::Grid<Elem> PSL::Grid<Elem>::clone() const {
  Grid<Elem> result(getWidth(), getHeight(), getDepth());
  memcpy((void *)result.getDataPtr(), (void *)this->getDataPtr(),
         sizeof(Elem) * getNbVoxels());
  return result;
}
...
void PSL::Grid<Elem>::saveAsDataFile(const std::string &fileName) {
  ...
  if (!outStream.good()) {
    PSL_THROW_EXCEPTION(
        "An error occured while writing the grid to a data file.")
  }
...
void PSL::Grid<Elem>::loadFromDataFile(const std::string &fileName) {
  ...
  if (elemSize != sizeof(Elem)) {
    PSL_THROW_EXCEPTION("Size of the datatype stored in the grid does not "
                        "match with the one from the file.")
  }
