...
namespace PSL {
template <typename T, typename U> DepthMap<T, U>::DepthMap() {
  width = 0;
  height = 0;
}
...
template <typename T, typename U> void DepthMap<T, U>::initialize(T value) {
  for (unsigned int i = 0; i < width * height; ++i)
    depths[i] = value;
}

template <typename T, typename U> T *DepthMap<T, U>::getDataPtr() {
  return &(depths[0]);
}

template <typename T, typename U> T &DepthMap<T, U>::operator()(int x, int y) {
...
template <typename T, typename U>
void DepthMap<T, U>::display(T minZ, T maxZ, int displayTime,
...
void DepthMap<T, U>::ComputeDepthMat(T minZ, T maxZ, cv::Mat &depth_inv_mat) {
  ...
  for (unsigned int y = 0; y < height; y++) {
    for (unsigned int x = 0; x < width; x++) {
      ...
      if (depth > 0) {
        invDepthsMat[y][x] =
            (1 / depthsMat[y][x] - 1 / maxZ) / (1 / minZ - 1 / maxZ);
      } else {
        invDepthsMat[y][x] = 0;
      }
    }
...
template <typename T, typename U>
void DepthMap<T, U>::ComputeColoredDepthMat(T minZ, T maxZ,
  ...
  for (int y = 0; y < colInvDepth.rows; y++) {
    ...
    for (int x = 0; x < colInvDepth.cols; ++x) {
      ...
      if (depth > 0) {
        int idx = (int)round(
            std::max((T)0, std::min(1 / depth - 1 / maxZ, 1 / minZ - 1 / maxZ) /
                               (1 / minZ - 1 / maxZ)) *
            (T)255);
  ...
  depth_inv_mat = colInvDepth;
}

template <typename T, typename U>
void DepthMap<T, U>::displayInvDepthColored(T minZ, T maxZ, int displayTime,
                                            const char *windowName) {
  cv::Mat colInvDepth = cv::Mat::zeros(height, width, CV_8UC3);
...
template <typename T, typename U>
void DepthMap<T, U>::meshToVRML(ofstream &os, string textureImageFileName,
                                float scale, float maxDispDiff, U maxDist) {
  if (!os.is_open()) {
  ...
  for (unsigned int y = 0; y < height; y++)
    for (unsigned int x = 0; x < width; x++)
      os << "               " << reprojPoints(x, y)(0) << " "
         << reprojPoints(x, y)(1) << " " << reprojPoints(x, y)(2) << ","
         << endl;
  os << "           ]" << endl;
  os << "       }" << endl;
  os << "       coordIndex [" << endl;
  for (unsigned int y = 0; y < height - 1; y++)
    for (unsigned int x = 0; x < width - 1; x++) {
      float dispDiff = (float)std::max(
          std::fabs(scale / (*this)(x, y) - scale / (*this)(x + 1, y)),
          std::max(
              std::fabs(scale / (*this)(x + 1, y) - scale / (*this)(x, y + 1)),
              std::fabs(scale / (*this)(x, y + 1)) - scale / (*this)(x, y)));
      if ((maxDispDiff < 0 || dispDiff < maxDispDiff) &&
          reprojPoints(x, y)(3) == 1 && reprojPoints(x + 1, y)(3) == 1 &&
          reprojPoints(x, y + 1)(3) == 1)
        os << "           " << y * width + x + 1 << ", " << y * width + x
           << ", " << (y + 1) * width + x << ", -1," << endl;
      dispDiff = (float)std::max(
          std::fabs(scale / (*this)(x + 1, y + 1) - scale / (*this)(x + 1, y)),
          std::max(
              std::fabs(scale / (*this)(x + 1, y) - scale / (*this)(x, y + 1)),
              std::fabs(scale / (*this)(x, y + 1)) -
                  scale / (*this)(x + 1, y + 1)));
      if ((maxDispDiff < 0 || dispDiff < maxDispDiff) &&
          reprojPoints(x + 1, y + 1)(3) == 1 &&
          reprojPoints(x + 1, y)(3) == 1 && reprojPoints(x, y + 1)(3) == 1)
        os << "           " << (y + 1) * width + x + 1 << ", "
  ...
  for (unsigned int y = 0; y < height; y++)
    for (unsigned int x = 0; x < width; x++)
      os << "                 " << (float)x / (float)width << " "
         << height - (float)y / (float)height << ", " << endl;
  os << "         ]" << endl;
  os << "     }" << endl;
  os << "   }" << endl;
  os << "}" << endl;
...
void DepthMap<T, U>::meshToVRML(ofstream &os, U maxDist, T maxDispDiff) {
  ...
  for (unsigned int y = 0; y < height - 1; y++)
    for (unsigned int x = 0; x < width - 1; x++) {
      float dispDiff = (float)std::max(
          std::fabs(1.0 / (*this)(x, y) - 1.0 / (*this)(x + 1, y)),
          std::max(std::fabs(1.0 / (*this)(x + 1, y) - 1.0 / (*this)(x, y + 1)),
                   std::fabs(1.0 / (*this)(x, y + 1)) - 1.0 / (*this)(x, y)));
      if ((maxDispDiff < 0 || dispDiff < maxDispDiff) &&
          reprojPoints(x, y)(3) == 1 && reprojPoints(x + 1, y)(3) == 1 &&
          reprojPoints(x, y + 1)(3) == 1)
        os << "           " << y * width + x + 1 << ", " << y * width + x
...
template <typename T, typename U>
void DepthMap<T, U>::saveInvDepthAsImage(string fileName, T minDepth,
                                         T maxDepth) {
  cv::Mat_<T> invDepthsMat(height, width);
  for (unsigned int y = 0; y < height; y++) {
    for (unsigned int x = 0; x < width; x++) {
      ...
      if (depth > 0) {
        invDepthsMat[y][x] =
            (1 / (*this)(x, y) - 1 / maxDepth) / (1 / minDepth - 1 / maxDepth);
      } else {
        invDepthsMat[y][x] = 0;
      }
    }
...
void DepthMap<T, U>::saveAsDataFile(string fileName) {
  ...
  // first unsigned int in an unsigned char because we know that char has always
  // size 1
  ...
  if (!outStream.good()) {
    PSL_THROW_EXCEPTION(
        "An error occured while writing out a depth map to a data file.")
  }
...
void DepthMap<T, U>::loadFromDataFile(string fileName) {
  ...
  if (endian != currentEndian) {
    PSL_THROW_EXCEPTION("Current platform does not have the same endian as the "
                        "depht map data file.")
  }
  ...
  if (uintSize != sizeof(unsigned int)) {
    PSL_THROW_EXCEPTION("Current platform does not have the same unsigned int "
                        "size as the one the file was written with.")
  }
  ...
  if (tSize != sizeof(T) || uSize != sizeof(U)) {
    PSL_THROW_EXCEPTION(
        "Sizes of the datatypes for sparse and dense geometry do not match.")
  }
...
template <typename T, typename U>
void DepthMap<T, U>::pointCloudColoredToVRML(ofstream &os, cv::Mat image,
                                             U maxDist) {
  if (!os.is_open()) {
    PSL_THROW_EXCEPTION("Outputstream not open.")
  }
  ...
  if (image.channels() == 1) {
    for (unsigned int y = 0; y < height; y++)
      for (unsigned int x = 0; x < width; x++) {
        os << "           " << image.at<unsigned char>(y, x) / 255.0 << " "
           << image.at<unsigned char>(y, x) / 255.0 << " "
           << image.at<unsigned char>(y, x) / 255.0 << "," << endl;
      }
  } else if (image.channels() == 3) {
    for (unsigned int y = 0; y < height; y++)
      for (unsigned int x = 0; x < width; x++) {
        os << "           " << image.at<unsigned char>(y, 3 * x + 2) / 255.0
           << " " << image.at<unsigned char>(y, 3 * x + 1) / 255.0 << " "
           << image.at<unsigned char>(y, 3 * x) / 255.0 << "," << endl;
      }
  }
  os << "         ]" << endl;
  os << "       }" << endl;
