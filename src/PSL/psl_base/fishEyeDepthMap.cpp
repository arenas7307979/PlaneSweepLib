...
template <typename T, typename U> FishEyeDepthMap<T, U>::FishEyeDepthMap() {
  width = 0;
  height = 0;
}

template <typename T, typename U>
FishEyeDepthMap<T, U>::FishEyeDepthMap(unsigned int width, unsigned int height,
                                       const FishEyeCameraMatrix<U> &cam) {
  this->width = width;
  this->height = height;
  this->cam = cam;
...
template <typename T, typename U> T *FishEyeDepthMap<T, U>::getDataPtr() {
  return &(depths[0]);
}
...
template <typename T, typename U>
void FishEyeDepthMap<T, U>::display(T minZ, T maxZ, int displayTime,
...
template <typename T, typename U>
void FishEyeDepthMap<T, U>::displayColored(T minZ, T maxZ, int displayTime,
                                           const char *windowName) {
  ...
  for (int y = 0; y < colDepth.rows; y++) {
    ...
    for (int x = 0; x < colDepth.cols; ++x) {
      ...
      if (depth > 0) {
        int idx = (int)round(std::max(
            (T)0, std::min((depth - minZ) / (maxZ - minZ), (T)1) * (T)255));
...
template <typename T, typename U>
void FishEyeDepthMap<T, U>::displayInvDepth(T minZ, T maxZ, int displayTime,
                                            const char *windowName) {
  cv::Mat_<T> depthsMat(height, width, getDataPtr());
...
template <typename T, typename U>
void FishEyeDepthMap<T, U>::displayInvDepthColored(T minZ, T maxZ,
                                                   int displayTime,
                                                   const char *windowName) {
...
template <typename T, typename U>
void FishEyeDepthMap<T, U>::pointCloudColoredToVRML(std::ofstream &os,
                                                    cv::Mat image, U maxDist) {
  if (!os.is_open()) {
    PSL_THROW_EXCEPTION("Outputstream not open.")
  }
    ...
    for (unsigned int y = 0; y < height; y++)
      for (unsigned int x = 0; x < width; x++) {
        os << "           " << image.at<unsigned char>(y, x) / 255.0 << " "
           ...
           << image.at<unsigned char>(y, x) / 255.0 << "," << std::endl;
      }
    ...
    for (unsigned int y = 0; y < height; y++)
      for (unsigned int x = 0; x < width; x++) {
        os << "           " << image.at<unsigned char>(y, 3 * x + 2) / 255.0
           ...
           << image.at<unsigned char>(y, 3 * x) / 255.0 << "," << std::endl;
      }
  }
  os << "         ]" << std::endl;
  os << "       }" << std::endl;
...
template <typename T, typename U>
void FishEyeDepthMap<T, U>::saveInvDepthAsColorImage(std::string fileName,
                                                     T minZ, T maxZ) {
...
template <typename T, typename U>
void FishEyeDepthMap<T, U>::meshToVRML(std::ofstream &os,
                                       std::string textureImageFileName,
                                       float scale, float maxDispDiff,
                                       U maxDist) {
  if (!os.is_open()) {
    PSL_THROW_EXCEPTION("Outputstream not open.")
  }
