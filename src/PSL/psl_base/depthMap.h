...
template <typename T, typename U> class DepthMap {
public:
  ...
  void ComputeColoredDepthMat(T minZ, T maxZ, cv::Mat &depth_inv_mat);
  ...
  void meshToVRML(ofstream &os, string textureImageFileName, float scale,
                  float maxDispDiff, U maxDist = -1);
