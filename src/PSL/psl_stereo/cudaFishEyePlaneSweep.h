...
void planeSweepComputeBestDepthsSubPixelDirect(
    DeviceBuffer<int> &bestPlanes, DeviceBuffer<float> &planeOffsets,
    int numPlanes, std::vector<float> &planes, float *bestDepthsAddr,
    int bestDepthsPitch, std::vector<float> &KrefInv, float XiRef);
}
}
...
public:
  ...
  void
  setSubPixelInterpolationMode(FishEyePlaneSweepSubPixelInterpMode interpMode);
...
private:
  ...
  void planeRT(const Eigen::Matrix<double, 4, 1> &plane,
               const FishEyeCameraMatrix<double> &refCam,
               const FishEyeCameraMatrix<double> &otherCam, float *RT);
  ...
  void matchImage(const CudaFishEyePlaneSweepImage &refImg,
                  ...
                  const Eigen::Matrix<double, 4, 1> &plane,
                  PSL_CUDA::DeviceBuffer<float> &costBuffer);
