...
CudaFishEyePlaneSweep::~CudaFishEyePlaneSweep() {
  ...
  for (std::map<int, CudaFishEyePlaneSweepImage>::iterator it = images.begin();
       it != images.end(); it++) {
...
void CudaFishEyePlaneSweep::setNumPlanes(int num) { this->numPlanes = num; }
...
void CudaFishEyePlaneSweep::setOcclusionMode(
...
void CudaFishEyePlaneSweep::setPlaneGenerationMode(
...
void CudaFishEyePlaneSweep::setMatchingCosts(
    FishEyePlaneSweepMatchingCosts matchingCosts) {
  this->matchingCosts = matchingCosts;
}

void CudaFishEyePlaneSweep::setScale(double scale) { this->scale = scale; }
...
int CudaFishEyePlaneSweep::addImage(cv::Mat image,
  ...
  if (scale != 1) {
    ...
    cv::Mat scaledImage;
    if (scale < 1)
      cv::resize(image, scaledImage, cv::Size(0, 0), scale, scale,
                 cv::INTER_AREA);
    else
      cv::resize(image, scaledImage, cv::Size(0, 0), scale, scale,
                 cv::INTER_LINEAR);
...
int CudaFishEyePlaneSweep::addDeviceImage(PSL_CUDA::DeviceImage &image,
                                          FishEyeCameraMatrix<double> &cam) {
  CudaFishEyePlaneSweepImage cPSI;
  cPSI.devImg = image;
  cPSI.cam = cam;
...
void CudaFishEyePlaneSweep::process(int refImgId,
  ...
  if (subPixelEnabled) {
    subPixelCostAccumBufferPrev1.reallocatePitched(refImg.devImg.getWidth(),
                                                   refImg.devImg.getHeight());
    subPixelCostAccumBufferPrev2.reallocatePitched(refImg.devImg.getWidth(),
                                                   refImg.devImg.getHeight());
    subPixelPlaneOffsetsBuffer.reallocatePitched(refImg.devImg.getWidth(),
                                                 refImg.devImg.getHeight());
  ...
  if (occlusionMode == FISH_EYE_PLANE_SWEEP_OCCLUSION_NONE) {
    ...
    int numBefore = 0;
    int numAfter = 0;
    for (std::map<int, CudaFishEyePlaneSweepImage>::iterator it =
             images.begin();
         it != images.end(); it++) {
  ...
  if (matchingCosts == FISH_EYE_PLANE_SWEEP_ZNCC) {
    ...
    planeSweepBoxFilterImageAndSqrImage(
        refImg.devImg, refImgBoxFilterBuffer, refImgSqrBoxFilterBuffer,
        boxFilterTempBuffer, boxFilterSqrTempBuffer, matchWindowRadiusX,
        matchWindowRadiusY);
  }
    ...
    for (std::map<int, CudaFishEyePlaneSweepImage>::iterator it =
      ...
      case FISH_EYE_PLANE_SWEEP_OCCLUSION_NONE: {
        ...
        case FISH_EYE_PLANE_SWEEP_SAD:
          planeSweepWarpADAccum(it->second.devImg, RT, KrefInvA,
                                (float)refImg.cam.getXi(), KotherA,
                                (float)it->second.cam.getXi(), refImg.devImg,
                                (float)accumScale0, costAccumBuffer);
          break;
        case FISH_EYE_PLANE_SWEEP_ZNCC:
          planeSweepWarpZNCCAccum(
              it->second.devImg, RT, KrefInvA, (float)refImg.cam.getXi(),
              KotherA, (float)it->second.cam.getXi(), refImg.devImg,
              refImgBoxFilterBuffer, refImgSqrBoxFilterBuffer,
              (float)accumScale0, costAccumBuffer, matchWindowRadiusX,
              matchWindowRadiusY);
          break;
        }
        break;
      ...
      case FISH_EYE_PLANE_SWEEP_OCCLUSION_BEST_K: {
        ...
        if (boxFilterBeforeOcclusion &&
            matchingCosts != FISH_EYE_PLANE_SWEEP_ZNCC) {
        ...
        case FISH_EYE_PLANE_SWEEP_SAD:
          if (it->first < refImgId) {
            planeSweepWarpADAccum(it->second.devImg, RT, KrefInvA,
                                  (float)refImg.cam.getXi(), KotherA,
                                  (float)it->second.cam.getXi(), refImg.devImg,
                                  (float)accumScale0, costAccumBeforeBuffer);
          } else {

            planeSweepWarpADAccum(it->second.devImg, RT, KrefInvA,
                                  (float)refImg.cam.getXi(), KotherA,
                                  (float)it->second.cam.getXi(), refImg.devImg,
                                  (float)accumScale1, costAccumAfterBuffer);
          }
          break;
        case FISH_EYE_PLANE_SWEEP_ZNCC:
          if (it->first < refImgId) {
            planeSweepWarpZNCCAccum(
                ...
                KotherA, (float)it->second.cam.getXi(), refImg.devImg,
                refImgBoxFilterBuffer, refImgSqrBoxFilterBuffer,
                (float)accumScale0, costAccumBeforeBuffer, matchWindowRadiusX,
                matchWindowRadiusY);
          } else {
            planeSweepWarpZNCCAccum(
                it->second.devImg, RT, KrefInvA, (float)refImg.cam.getXi(),
                KotherA, (float)it->second.cam.getXi(), refImg.devImg,
                refImgBoxFilterBuffer, refImgSqrBoxFilterBuffer,
                (float)accumScale1, costAccumAfterBuffer, matchWindowRadiusX,
                matchWindowRadiusY);
          }
          break;
        }
  ...
  if (outputBestDepthEnabled) {
    ...
    bestDepth = FishEyeDepthMap<float, double>(
        refImg.devImg.getWidth(), refImg.devImg.getHeight(), refImg.cam);
    ...
    if (subPixelEnabled) {
      ...
      case FISH_EYE_PLANE_SWEEP_SUB_PIXEL_INTERP_INVERSE:
        planeSweepComputeBestDepthsSubPixelInverse(
            ...
            KrefInvVec, (float)refImg.cam.getXi());
        break;
...
void CudaFishEyePlaneSweep::planeRT(const Eigen::Matrix<double, 4, 1> &plane,
                                    const FishEyeCameraMatrix<double> &refCam,
                                    const FishEyeCameraMatrix<double> &otherCam,
                                    float *RT) {
  ...
  Eigen::Matrix<double, 3, 3> RTmat =
      rRel + ((double)1) / plane(3) * tRel * n.transpose();

  //    std::cout << RTmat << std::endl;

  RT[0] = (float)RTmat(0, 0);
  RT[1] = (float)RTmat(0, 1);
  RT[2] = (float)RTmat(0, 2);
  RT[3] = (float)RTmat(1, 0);
  RT[4] = (float)RTmat(1, 1);
  RT[5] = (float)RTmat(1, 2);
  RT[6] = (float)RTmat(2, 0);
  RT[7] = (float)RTmat(2, 1);
  RT[8] = (float)RTmat(2, 2);
}

void CudaFishEyePlaneSweep::eigenMat3dToInlineArryf(const Eigen::Matrix3d &mat,
                                                    float *arr) {
  arr[0] = (float)mat(0, 0);
  arr[1] = (float)mat(0, 1);
  arr[2] = (float)mat(0, 2);
...
void CudaFishEyePlaneSweep::matchImage(
    const CudaFishEyePlaneSweepImage &refImg,
  ...
  switch (matchingCosts) {
  case FISH_EYE_PLANE_SWEEP_SAD:
    planeSweepWarpAD(otherImg.devImg, RT, KrefInvA, (float)refImg.cam.getXi(),
                     KotherA, (float)otherImg.cam.getXi(), refImg.devImg,
                     costBuffer);
    break;
  case FISH_EYE_PLANE_SWEEP_ZNCC:
    planeSweepWarpZNCC(otherImg.devImg, RT, KrefInvA, (float)refImg.cam.getXi(),
                       KotherA, (float)otherImg.cam.getXi(), refImg.devImg,
                       refImgBoxFilterBuffer, refImgSqrBoxFilterBuffer,
...
Grid<float> CudaFishEyePlaneSweep::getBestCosts() { return bestCosts; }

Grid<float> CudaFishEyePlaneSweep::getUniquenessRatios() {
  return uniqunessRatios;
}

void CudaFishEyePlaneSweep::updateCameraMatrix(
    int id, FishEyeCameraMatrix<double> &cam) {
...
void CudaFishEyePlaneSweep::setSubPixelInterpolationMode(
    FishEyePlaneSweepSubPixelInterpMode interpMode) {
