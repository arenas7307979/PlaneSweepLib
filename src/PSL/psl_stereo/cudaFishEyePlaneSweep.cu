...
__forceinline__ __device__ float computeWarpedGrayscaleTexturePixel(
    int x, int y, int width, int height, float krefinv11, float krefinv13,
    float krefinv22, float krefinv23, float xiref, float kother11,
    float kother13, float kother22, float kother23, float xiother, float rt11,
    float rt12, float rt13, float rt21, float rt22, float rt23, float rt31,
    float rt32, float rt33);
__forceinline__ __device__ float computeZNCC(float normalizer, float ref,
                                             float refSqr, float other,
                                             float otherSqr, float prod);
...
void planeSweepInitTexturing() {
  if (!planeSweepTexturesInitialized) {
    ...
    planeSweepColorChannelDesc =
        cudaCreateChannelDesc(8, 8, 8, 8, cudaChannelFormatKindUnsigned);
    planeSweepGrayscaleChannelDesc =
...
__global__ void boxFilterCostsKernel(DeviceBuffer<float> filteredBuf,
                                     int radius_x, int radius_y) {
  // implemented according to "Stereo Imaging with CUDA" and lots of changes
...
void planeSweepBoxFilterCosts(DeviceBuffer<float> &costBuf,
                              DeviceBuffer<float> &filteredCostBuf,
                              int radius_x, int radius_y) {
  // Bind texture
  ...
  dim3 gridDim(getNumTiles(filteredCostBuf.getWidth(),
                           PLANE_SWEEP_BOX_FILTER_NUM_THREADS - 2 * radius_x),
               getNumTiles(filteredCostBuf.getHeight(),
                           PLANE_SWEEP_BOX_FILTER_ROWS_PER_THREAD));
  dim3 blockDim(PLANE_SWEEP_BOX_FILTER_NUM_THREADS, 1);

  // run box filtering kernel
  boxFilterCostsKernel<<<gridDim, blockDim, sharedMemSize>>>(
      filteredCostBuf, radius_x, radius_y);
...
__global__ void
planeSweepBoxFilterImageAndSqrImageKernel(DeviceBuffer<float> boxFilterBuf,
                                          DeviceBuffer<float> boxFilterSqrBuf,
                                          int radius_x, int radius_y) {
  // implemented according to "Stereo Imaging with CUDA" and lots of changes
...
void planeSweepBoxFilterImageAndSqrImage(const DeviceImage &refImg,
                                         DeviceBuffer<float> &boxFilterBuf,
                                         DeviceBuffer<float> &boxFilterSqrBuf,
                                         DeviceBuffer<float> &tempBuf,
                                         DeviceBuffer<float> &tempSqrBuf,
                                         int radius_x, int radius_y) {
  ...
  const int sharedMemSize =
      2 * (PLANE_SWEEP_BOX_FILTER_NUM_THREADS) * sizeof(float);

  dim3 gridDim(
      getNumTiles(width, PLANE_SWEEP_BOX_FILTER_NUM_THREADS - 2 * radius_x),
      getNumTiles(height, PLANE_SWEEP_BOX_FILTER_ROWS_PER_THREAD));
  dim3 blockDim(PLANE_SWEEP_BOX_FILTER_NUM_THREADS, 1);
  planeSweepBoxFilterImageAndSqrImageKernel<<<gridDim, blockDim,
                                              sharedMemSize>>>(
...
__global__ void planeSweepZNCCKernel(
    float krefinv11, float krefinv13, float krefinv22, float krefinv23,
    float xiref, float kother11, float kother13, float kother22, float kother23,
    float xiother, float rt11, float rt12, float rt13, float rt21, float rt22,
    float rt23, float rt31, float rt32, float rt33,
    DeviceBuffer<float> refFiltered, DeviceBuffer<float> refSqrFiltered,
    DeviceBuffer<float> costBuffer, int radius_x, int radius_y,
    float normalizerZNCC) {
  // implemented according to "Stereo Imaging with CUDA" and lots of changes
  // since initial implementation
  ...
  const int X =
      (blockIdx.x * PLANE_SWEEP_BOX_FILTER_NUM_THREADS + threadIdx.x) -
      blockIdx.x * 2 * radius_x;
  const int Y = blockIdx.y * PLANE_SWEEP_BOX_FILTER_ROWS_PER_THREAD;
  ...
  if (X < (width + 2 * radius_x) && Y < height) {
    ...
    for (int i = 0; i <= 2 * radius_y; i++) {
      ...
      const float otherVal = computeWarpedGrayscaleTexturePixel(
          x_img, y_img, width, height, krefinv11, krefinv13, krefinv22,
          krefinv23, xiref, kother11, kother13, kother22, kother23, xiother,
    ...
    if (threadIdx.x + 2 * radius_x < PLANE_SWEEP_BOX_FILTER_NUM_THREADS &&
      ...
      costBuffer(X, Y) =
          computeZNCC(normalizerZNCC, ref, refSqr, other, otherSqr, prod);
    }
    ...
    for (int row = 1;
      ...
      const float addOtherVal = computeWarpedGrayscaleTexturePixel(
          x_img, y_img + 2 * radius_y + 1, width, height, krefinv11, krefinv13,
          krefinv22, krefinv23, xiref, kother11, kother13, kother22, kother23,
          xiother, rt11, rt12, rt13, rt21, rt22, rt23, rt31, rt32, rt33);
...
void planeSweepWarpZNCC(const DeviceImage &otherImg, float *RT,
                        ...
                        DeviceBuffer<float> &refImgBoxFilterBuf,
                        DeviceBuffer<float> &refImgSqrBoxFilterBuf,
                        DeviceBuffer<float> &costBuf, int radius_x,
                        int radius_y) {
  // bind textures
  PSL_CUDA_CHECKED_CALL(
  ...
  const int sharedMemSize =
      3 * (PLANE_SWEEP_BOX_FILTER_NUM_THREADS) * sizeof(float);
  dim3 gridDim(
      getNumTiles(refImg.getWidth(),
                  PLANE_SWEEP_BOX_FILTER_NUM_THREADS - 2 * radius_x),
      getNumTiles(refImg.getHeight(), PLANE_SWEEP_BOX_FILTER_ROWS_PER_THREAD));
  dim3 blockDim(PLANE_SWEEP_BOX_FILTER_NUM_THREADS, 1);

  const float normalizerZNCC =
      1.0f / ((float)(2 * radius_x + 1) * (2 * radius_y + 1));

  // run filter kernel
  planeSweepZNCCKernel<<<gridDim, blockDim, sharedMemSize>>>(
      KRefInv[0], KRefInv[2], KRefInv[4], KRefInv[5], XiRef, Kother[0],
      Kother[2], Kother[4], Kother[5], XiOther, RT[0], RT[1], RT[2], RT[3],
      RT[4], RT[5], RT[6], RT[7], RT[8], refImgBoxFilterBuf,
      refImgSqrBoxFilterBuf, costBuf, radius_x, radius_y, normalizerZNCC);
  PSL_CUDA_CHECK_ERROR
...
__global__ void planeSweepZNCCAccumKernel(
    ...
    float rt23, float rt31, float rt32, float rt33,
    DeviceBuffer<float> refFiltered, DeviceBuffer<float> refSqrFiltered,
    DeviceBuffer<float> costAccumBuffer, float accumScale, int radius_x,
    int radius_y, float normalizerZNCC) {
  // implemented according to "Stereo Imaging with CUDA" and lots of changes
  // since initial implementation
...
void planeSweepWarpZNCCAccum(
    const DeviceImage &otherImg, float *RT, const float *KRefInv,
    const float XiRef, const float *Kother, const float XiOther,
    const DeviceImage &refImg, DeviceBuffer<float> &refImgBoxFilterBuf,
    DeviceBuffer<float> &refImgSqrBoxFilterBuf, float accumScale,
    DeviceBuffer<float> &costAccumBuf, int radius_x, int radius_y) {
  // bind textures
  PSL_CUDA_CHECKED_CALL(
      cudaBindTexture2D(0, planeSweepGrayscaleTexture, otherImg.getAddr(),
                        planeSweepGrayscaleChannelDesc, otherImg.getWidth(),
  ...
  // run filter kernel
  planeSweepZNCCAccumKernel<<<gridDim, blockDim, sharedMemSize>>>(
      KRefInv[0], KRefInv[2], KRefInv[4], KRefInv[5], XiRef, Kother[0],
      Kother[2], Kother[4], Kother[5], XiOther, RT[0], RT[1], RT[2], RT[3],
      RT[4], RT[5], RT[6], RT[7], RT[8], refImgBoxFilterBuf,
      refImgSqrBoxFilterBuf, costAccumBuf, accumScale, radius_x, radius_y,
      normalizerZNCC);
  PSL_CUDA_CHECK_ERROR
...
__global__ void planeSweepWarpADGrayscaleKernel(
    const int srcImgWidth, const int srcImgHeight, float krefinv11,
    float krefinv13, float krefinv22, float krefinv23, float xiref,
    float kother11, float kother13, float kother22, float kother23,
    float xiother, float rt11, float rt12, float rt13, float rt21, float rt22,
    float rt23, float rt31, float rt32, float rt33, DeviceImage refImg,
...
void planeSweepWarpAD(const DeviceImage &srcImg, const float *RT,
  ...
  dim3 gridDim(getNumTiles(refImg.getWidth(), PLANE_SWEEP_TILE_WIDTH),
               getNumTiles(refImg.getHeight(), PLANE_SWEEP_TILE_HEIGHT));
  ...
  planeSweepWarpADGrayscaleKernel<<<gridDim, blockDim>>>(
      srcImg.getWidth(), srcImg.getHeight(), KRefInv[0], KRefInv[2], KRefInv[4],
      KRefInv[5], XiRef, Kother[0], Kother[2], Kother[4], Kother[5], XiOther,
      RT[0], RT[1], RT[2], RT[3], RT[4], RT[5], RT[6], RT[7], RT[8], refImg,
      costBuf);
  PSL_CUDA_CHECK_ERROR
  PSL_CUDA_CHECKED_CALL(cudaUnbindTexture(planeSweepGrayscaleTexture);)
}

__global__ void planeSweepWarpADAccumGrayscaleKernel(
    const int srcImgWidth, const int srcImgHeight, float krefinv11,
    float krefinv13, float krefinv22, float krefinv23, float xiref,
    float kother11, float kother13, float kother22, float kother23,
    float xiother, float rt11, float rt12, float rt13, float rt21, float rt22,
...
void planeSweepWarpADAccum(DeviceImage &srcImg, const float *RT,
                           const float *KRefInv, const float XiRef,
                           const float *Kother, const float XiOther,
                           DeviceImage &refImg, float accumScale,
  ...
  planeSweepWarpADAccumGrayscaleKernel<<<gridDim, blockDim>>>(
      srcImg.getWidth(), srcImg.getHeight(), KRefInv[0], KRefInv[2], KRefInv[4],
      KRefInv[5], XiRef, Kother[0], Kother[2], Kother[4], Kother[5], XiOther,
      RT[0], RT[1], RT[2], RT[3], RT[4], RT[5], RT[6], RT[7], RT[8], refImg,
      accumScale, costAccumBuf);
  PSL_CUDA_CHECK_ERROR
  PSL_CUDA_CHECKED_CALL(cudaUnbindTexture(planeSweepGrayscaleTexture);)
}

__global__ void planeSweepUpdateBestKKernel(DeviceBuffer<float> newCosts,
                                            DeviceBuffer<float> bestCosts,
                                            DeviceBuffer<float> bestMin) {
...
void updateBestK(DeviceBuffer<float> &newCostsBuf,
                 DeviceBuffer<float> &bestCostsBuf,
                 DeviceBuffer<float> &bestMinBuf) {
  dim3 gridDim(getNumTiles(bestCostsBuf.getWidth(), PLANE_SWEEP_TILE_WIDTH),
               getNumTiles(bestCostsBuf.getHeight(), PLANE_SWEEP_TILE_HEIGHT));
  dim3 blockDim(PLANE_SWEEP_TILE_WIDTH, PLANE_SWEEP_TILE_HEIGHT);

  planeSweepUpdateBestKKernel<<<gridDim, blockDim>>>(newCostsBuf, bestCostsBuf,
                                                     bestMinBuf);
  PSL_CUDA_CHECK_ERROR
}

__global__ void planeSweepAccumCostKernelBestK(DeviceBuffer<float> accumBuf,
                                               DeviceBuffer<float> costBuf,
                                               DeviceBuffer<float> minCostBuf,
                                               float maxVal, float accumScale) {
  // get position of outupt
  const unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
  const unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
...
void planeSweepAccumCostBestK(DeviceBuffer<float> &costAccumBuf,
                              DeviceBuffer<float> &costBuf,
                              DeviceBuffer<float> &minCostBuf, float maxVal,
                              float accumScale) {
  dim3 gridDim(getNumTiles(costAccumBuf.getWidth(), PLANE_SWEEP_TILE_WIDTH),
               getNumTiles(costAccumBuf.getHeight(), PLANE_SWEEP_TILE_HEIGHT));
  dim3 blockDim(PLANE_SWEEP_TILE_WIDTH, PLANE_SWEEP_TILE_HEIGHT);

  planeSweepAccumCostKernelBestK<<<gridDim, blockDim>>>(
      costAccumBuf, costBuf, minCostBuf, maxVal, accumScale);
  PSL_CUDA_CHECK_ERROR
}
...
__global__ void planeSweepUpdateBestPlaneSubPixelKernel(
  ...
  if (x < currCosts.getWidth() && y < currCosts.getHeight()) {
    ...
    if (prev1_t <= bestPlaneCosts(x, y) && prev2_t > prev1(x, y) &&
        currCosts_t > prev1(x, y)) {
      // parabola
...
__global__ void planeSweepUpdateBestAndSecondBestPlaneSubPixelKernel(
  ...
  if (x < currCosts.getWidth() && y < currCosts.getHeight()) {
    ...
    // we only comput the parabola if we found a new minimal cost
    if (prev1_t <= bestPlaneCosts(x, y) && prev2_t > prev1_t &&
        currCosts_t > prev1_t) {
      // parabola
      const float denom = currCosts(x, y) + prev2(x, y) - 2 * prev1(x, y);
      float offset = 0.0f;
...
__global__ void planeSweepMinFloatKernel(DeviceBuffer<float> buf1,
                                         DeviceBuffer<float> buf2,
                                         DeviceBuffer<float> bufMin) {
  // get position of outupt
  unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
...
void planeSweepMinFloat(DeviceBuffer<float> &buf1, DeviceBuffer<float> &buf2,
  ...
  dim3 gridDim(getNumTiles(buf1.getWidth(), PLANE_SWEEP_TILE_WIDTH),
               getNumTiles(buf1.getHeight(), PLANE_SWEEP_TILE_HEIGHT));
  dim3 blockDim(PLANE_SWEEP_TILE_WIDTH, PLANE_SWEEP_TILE_HEIGHT);
...
__global__ void uniqunessRatioKernel(DeviceBuffer<float> bestCost,
                                     DeviceBuffer<float> secondBestCost,
                                     DeviceBuffer<float> uniquenessRatios) {
  unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
...
void computeUniquenessRatio(DeviceBuffer<float> &bestCost,
                            DeviceBuffer<float> &secondBestCost,
                            DeviceBuffer<float> &uniquenessRatios) {
  dim3 gridDim(getNumTiles(bestCost.getWidth(), PLANE_SWEEP_TILE_WIDTH),
               getNumTiles(bestCost.getHeight(), PLANE_SWEEP_TILE_HEIGHT));
  dim3 blockDim(PLANE_SWEEP_TILE_WIDTH, PLANE_SWEEP_TILE_HEIGHT);

  uniqunessRatioKernel<<<gridDim, blockDim>>>(bestCost, secondBestCost,
                                              uniquenessRatios);
  PSL_CUDA_CHECK_ERROR
}

__global__ void planeSweepComputeBestDepthsKernel(
    DeviceBuffer<int> bestPlanes, float *planesAddr, size_t planesPitch,
    float *bestDepthsDAddr, size_t bestDepthsDPitch, float3 KrefInvCol1,
    float3 KrefInvCol2, float3 KrefInvCol3, float XiRef) {
  unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  if (x < bestPlanes.getWidth() && y < bestPlanes.getHeight()) {
    ...
    const float planeD =
        *((float *)((char *)planesAddr + 3 * planesPitch) + planeIdx);

    const float denom = xx * planeN.x + yy * planeN.y + planeN.z;

    *((float *)((char *)bestDepthsDAddr + y * bestDepthsDPitch) + x) =
        -planeD / denom;
  }
}

void planeSweepComputeBestDepths(DeviceBuffer<int> &bestPlanes, int numPlanes,
                                 std::vector<float> &planes,
                                 float *bestDepthsAddr, int bestDepthsPitch,
                                 std::vector<float> &KrefInv, float XiRef) {
  ...
  size_t planesPitch;
  PSL_CUDA_CHECKED_CALL(
      cudaMallocPitch(&planesAddr, &planesPitch, sizeof(float) * numPlanes, 4);)
  // copy planes to device
  PSL_CUDA_CHECKED_CALL(cudaMemcpy2D(planesAddr, planesPitch, &(planes[0]),
                                     sizeof(float) * numPlanes,
                                     sizeof(float) * numPlanes, 4,
                                     cudaMemcpyHostToDevice);)

  // allocate memory on device for best depths
  float *bestDepthsDAddr;
  size_t bestDepthsDPitch;
  PSL_CUDA_CHECKED_CALL(cudaMallocPitch(&bestDepthsDAddr, &bestDepthsDPitch,
  ...
  planeSweepComputeBestDepthsKernel<<<gridDim, blockDim>>>(
      bestPlanes, planesAddr, planesPitch, bestDepthsDAddr, bestDepthsDPitch,
      KrefInvCol1, KrefInvCol2, KrefInvCol3, XiRef);
  PSL_CUDA_CHECK_ERROR

  // download result from gpu
  PSL_CUDA_CHECKED_CALL(cudaMemcpy2D(bestDepthsAddr, bestDepthsPitch,
                                     bestDepthsDAddr, bestDepthsDPitch,
                                     sizeof(float) * width, height,
...
__global__ void planeSweepComputeBestDepthsSubPixelInverseKernel(
    DeviceBuffer<int> bestPlanes, DeviceBuffer<float> subPixelPlaneOffsets,
    float *planesAddr, size_t planesPitch, int numPlanes,
    float *bestDepthsDAddr, size_t bestDepthsDPitch, float3 KrefInvCol1,
    float3 KrefInvCol2, float3 KrefInvCol3, float XiRef) {
  unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  if (x < bestPlanes.getWidth() && y < bestPlanes.getHeight()) {
    ...
    if (subPixelPlaneOffsets(x, y) < 0) {
      ...
      if (oPlaneIdx >= 0) {
        float oPlaneD =
            *((float *)((char *)planesAddr + 3 * planesPitch) + oPlaneIdx);

        float dStep =
            (1.0f / oPlaneD - 1.0f / planeD) * subPixelPlaneOffsets(x, y);
    ...
    if (subPixelPlaneOffsets(x, y) > 0) {
      ...
      if (oPlaneIdx < numPlanes) {
        ...
        float dStep =
            (1.0f / planeD - 1.0f / oPlaneD) * subPixelPlaneOffsets(x, y);
        planeD = 1.0f / (1.0f / planeD - dStep);
      }
    }
...
void planeSweepComputeBestDepthsSubPixelInverse(
    DeviceBuffer<int> &bestPlanes, DeviceBuffer<float> &subPixelPlaneOffsets,
    int numPlanes, std::vector<float> &planes, float *bestDepthsAddr,
    int bestDepthsPitch, std::vector<float> &KrefInv, float XiRef) {
  const int width = bestPlanes.getWidth();
  const int height = bestPlanes.getHeight();
  ...
  planeSweepComputeBestDepthsSubPixelInverseKernel<<<gridDim, blockDim>>>(
      bestPlanes, subPixelPlaneOffsets, planesAddr, planesPitch, numPlanes,
...
__global__ void planeSweepComputeBestDepthsSubPixelDirectKernel(
    DeviceBuffer<int> bestPlanes, DeviceBuffer<float> subPixelPlaneOffsets,
    float *planesAddr, size_t planesPitch, int numPlanes,
    float *bestDepthsDAddr, size_t bestDepthsDPitch, float3 KrefInvCol1,
    float3 KrefInvCol2, float3 KrefInvCol3, float XiRef) {
...
void planeSweepComputeBestDepthsSubPixelDirect(
    DeviceBuffer<int> &bestPlanes, DeviceBuffer<float> &subPixelPlaneOffsets,
    int numPlanes, std::vector<float> &planes, float *bestDepthsAddr,
    int bestDepthsPitch, std::vector<float> &KrefInv, float XiRef) {
  const int width = bestPlanes.getWidth();
  ...
  planeSweepComputeBestDepthsSubPixelDirectKernel<<<gridDim, blockDim>>>(
      bestPlanes, subPixelPlaneOffsets, planesAddr, planesPitch, numPlanes,
      bestDepthsDAddr, bestDepthsDPitch, KrefInvCol1, KrefInvCol2, KrefInvCol3,
      XiRef);
  PSL_CUDA_CHECK_ERROR
...
__forceinline__ __device__ float computeWarpedGrayscaleTexturePixel(
  ...
  const float fact =
      (xiref + sqrtf(abs(1 + (1 - xiref * xiref) * mxPmySqr))) / (mxPmySqr + 1);
