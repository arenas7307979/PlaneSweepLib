...
__forceinline__ __device__ float computeWarpedGrayscaleTexturePixel(
    int x, int y, int width, int height, float h11, float h12, float h13,
...
__global__ void planeSweepWarpZNCCKernel(
    float h11, float h12, float h13, float h21, float h22, float h23, float h31,
    float h32, float h33, DeviceBuffer<float> refFiltered,
    DeviceBuffer<float> refSqrFiltered, DeviceBuffer<float> costBuffer,
    int radius_x, int radius_y, float normalizerZNCC) {
  // Implemented according to "Stereo Imaging with CUDA" and lots of change
  // since initial implementation
...
void planeSweepWarpZNCC(const DeviceImage &otherImg, float *homography,
  ...
  // run filter kernel
  planeSweepWarpZNCCKernel<<<gridDim, blockDim, sharedMemSize>>>(
      homography[0], homography[1], homography[2], homography[3], homography[4],
      homography[5], homography[6], homography[7], homography[8],
      refImgBoxFilterBuf, refImgSqrBoxFilterBuf, costBuf, radius_x, radius_y,
      normalizerZNCC);
...
__global__ void planeSweepZNCCAccumKernel(
    float h11, float h12, float h13, float h21, float h22, float h23, float h31,
    float h32, float h33, DeviceBuffer<float> refFiltered,
    DeviceBuffer<float> refSqrFiltered, DeviceBuffer<float> costAccumBuffer,
    float accumScale, int radius_x, int radius_y, float normalizerZNCC) {
  // Implemented according to "Stereo Imaging with CUDA" and lots of changes
  // since the initial implementation
...
void planeSweepWarpZNCCAccum(const DeviceImage &otherImg, float *homography,
                             ...
                             DeviceBuffer<float> &refImgSqrBoxFilterBuf,
                             float accumScale,
                             DeviceBuffer<float> &costAccumBuf, int radius_x,
                             int radius_y) {
  // bind textures
  PSL_CUDA_CHECKED_CALL(
      cudaBindTexture2D(0, g_gray_scale_texture, otherImg.getAddr(),
  ...
  planeSweepZNCCAccumKernel<<<gridDim, blockDim, sharedMemSize>>>(
      homography[0], homography[1], homography[2], homography[3], homography[4],
      homography[5], homography[6], homography[7], homography[8],
      refImgBoxFilterBuf, refImgSqrBoxFilterBuf, costAccumBuf, accumScale,
      radius_x, radius_y, normalizerZNCC);
  PSL_CUDA_CHECK_ERROR
...
void planeSweepWarpAD(const DeviceImage &srcImg, bool color,
                      const float *homography, const DeviceImage &refImg,
  ...
  if (color) {
    ...
    PSL_CUDA_CHECKED_CALL(
        cudaBindTexture2D(0, planeSweepColorTexture, srcImg.getAddr(),
                          planeSweepColorChannelDesc, srcImg.getWidth(),
                          srcImg.getHeight(), srcImg.getPitch());)

    planeSweepWarpADColorKernel<<<gridDim, blockDim>>>(
        srcImg.getWidth(), srcImg.getHeight(), homography[0], homography[1],
...
void planeSweepWarpADAccum(DeviceImage &srcImg, bool color,
                           const float *homography, DeviceImage &refImg,
                           float accumScale,
  ...
  if (color) {
    ...
    planeSweepWarpADAccumColorKernel<<<gridDim, blockDim>>>(
        srcImg.getWidth(), srcImg.getHeight(), homography[0], homography[1],
        homography[2], homography[3], homography[4], homography[5],
...
__global__ void planeSweepComputeBestDepthsKernel(
    ...
    float3 KrefInvCol2, float3 KrefInvCol3) {
  unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  if (x < bestPlanes.getWidth() && y < bestPlanes.getHeight()) {
    ...
    float3 KrefInvTn;
    KrefInvTn.x = KrefInvCol1.x * planeN.x + KrefInvCol1.y * planeN.y +
                  KrefInvCol1.z * planeN.z;
    KrefInvTn.y = KrefInvCol2.x * planeN.x + KrefInvCol2.y * planeN.y +
                  KrefInvCol2.z * planeN.z;
    KrefInvTn.z = KrefInvCol3.x * planeN.x + KrefInvCol3.y * planeN.y +
                  KrefInvCol3.z * planeN.z;
    ...
    const float denom =
        KrefInvTn.x * pos.x + KrefInvTn.y * pos.y + KrefInvTn.z * pos.z;
...
void planeSweepComputeBestDepths(DeviceBuffer<int> &bestPlanes, int numPlanes,
                                 vector<float> &planes, float *bestDepthsAddr,
                                 int bestDepthsPitch, vector<float> &KrefInv) {
  ...
  planeSweepComputeBestDepthsKernel<<<gridDim, blockDim>>>(
      ...
      KrefInvCol1, KrefInvCol2, KrefInvCol3);
  PSL_CUDA_CHECK_ERROR
...
void planeSweepComputeBestDepthsSubPixelInverse(
    DeviceBuffer<int> &bestPlanes, DeviceBuffer<float> &subPixelPlaneOffsets,
    int numPlanes, vector<float> &planes, float *bestDepthsAddr,
    int bestDepthsPitch, vector<float> &KrefInv) {
  const int width = bestPlanes.getWidth();
  const int height = bestPlanes.getHeight();
