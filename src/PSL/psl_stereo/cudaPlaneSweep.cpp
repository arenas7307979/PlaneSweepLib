...
int CudaPlaneSweep::addDeviceImage(DeviceImage &devImage,
...
void CudaPlaneSweep::setNumPlanes(int num) { this->numPlanes = num; }

void CudaPlaneSweep::setOcclusionMode(PlaneSweepOcclusionMode occlusionMode) {
  this->occlusionMode = occlusionMode;
}

void CudaPlaneSweep::setOcclusionBestK(int bestK) { occlusionBestK = bestK; }
...
void CudaPlaneSweep::ComputeHomographyFromReferenceImage(
  ...
  Matrix<double, 3, 3> Hmat =
      kOther * (rRel + ((double)1) / plane(3) * tRel * n.transpose()) *
      kRef.inverse();
...
DepthMap<float, double> CudaPlaneSweep::getBestDepth() { return bestDepth; }

CameraMatrix<double> CudaPlaneSweep::getRefImgCam() { return refImgCam; }

Grid<float> CudaPlaneSweep::getBestCosts() { return bestCosts; }
...
void CudaPlaneSweep::matchImage(const CudaPlaneSweepImage &refImg,
  ...
  switch (matchingCosts) {
  case PLANE_SWEEP_SAD:
    planeSweepWarpAD(otherImg.devImg, colorMatchingEnabled, H, refImg.devImg,
                     costBuffer);
    break;
  case PLANE_SWEEP_ZNCC:
    planeSweepWarpZNCC(otherImg.devImg, H, refImg.devImg, refImgBoxFilterBuffer,
                       refImgSqrBoxFilterBuffer, costBuffer, matchWindowRadiusX,
                       matchWindowRadiusY);
    break;
  }
}

void CudaPlaneSweep::setPlaneGenerationMode(
...
void CudaPlaneSweep::setScale(double scale) { this->scale = scale; }

void CudaPlaneSweep::enableOuputUniquenessRatio(bool enabled) {
  this->outputUniquenessRatioEnabled = enabled;
}

Grid<float> CudaPlaneSweep::getUniquenessRatios() { return uniqunessRatios; }

void CudaPlaneSweep::enableOutputCostVolume(bool enabled) {
  this->outputCostVolumeEnabled = enabled;
}

Grid<float> CudaPlaneSweep::getCostVolume() { return costVolume; }

Grid<Vector4d> CudaPlaneSweep::getPlanes() { return planes; }

void CudaPlaneSweep::setSubPixelInterpolationMode(
    PlaneSweepSubPixelInterpMode interpMode) {
